...
__global__ void SetArrayZeros(TNum *arr, uint32_t length) {
...
__global__ void GaussFirst(TNum *a, TNum *b, uint32_t row, uint32_t shift) {
	...
	for (curr.Row = begin.Row + row + 1; curr.Row < *SIZE_N; curr.Row += offset.Row) {
		...
		for (curr.Col = begin.Col; curr.Col < *SIZE_K; curr.Col += offset.Col) {
...
__global__ void GaussSecond(TNum *a, TNum *b, uint32_t row, uint32_t shift) {
	...
	for (curr.Row = row - 1 - begin.Row; curr.Row >= 0; curr.Row -= offset.Row) {
